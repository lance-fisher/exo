...
REM  removes the github-cleanup folder
REM =============================================
setlocal enabledelayedexpansion

set CLEANUP_DIR=D:\ProjectsHome\github-cleanup
...
echo    5. Remove all cleanup scripts
echo.
...
set COORD=%CLEANUP_DIR%\.doctor-results.json
echo {> "%COORD%"
...
del /f /q "%CLEANUP_DIR%\.cleanup-progress.txt" 2>nul

REM Delete scripts 1-3 and supporting files
del /f /q "%CLEANUP_DIR%\1-cleanup-stale-branches.bat" 2>nul
if not errorlevel 1 echo    Removed 1-cleanup-stale-branches.bat
del /f /q "%CLEANUP_DIR%\2-install-github-cli.bat" 2>nul
if not errorlevel 1 echo    Removed 2-install-github-cli.bat
del /f /q "%CLEANUP_DIR%\3-extract-projects.bat" 2>nul
if not errorlevel 1 echo    Removed 3-extract-projects.bat
del /f /q "%CLEANUP_DIR%\doctor-coordination.json" 2>nul
del /f /q "%CLEANUP_DIR%\README.txt" 2>nul
del /f /q "%CLEANUP_DIR%\.doctor-results.json" 2>nul

echo.
echo  NOTE: The health report is saved at:
echo    %REPORT%
echo.
echo  ========================================
...
echo  will also delete itself and the
echo  github-cleanup folder)...
pause >nul

REM Self-delete and remove folder (must be last)
(goto) 2>nul & del "%~f0" & rmdir /q "%CLEANUP_DIR%" 2>nul
